...
<img src="/../public/images/2023-03-08-Net%20Working/LinuxIPNetworkingaction=AttachFile&do=get&target=i_example.png" style="zoom:67%;" />
...
<img src="/../public/images/2023-03-08-Net%20Working/LinuxIPNetworkingaction=AttachFile&do=get&target=o_path.png"  />
...
Along the way, each socket and protocol performs various checks and formatting functions, detailed in later chapters. The entire process is implemented with references and jump tables that isolate each protocol, most of which are set up during initialization when the computer boots. See [Chapter 3](https://kernelnewbies.org/Documents/LinuxIPNetworking#chapter3) for details of the initialization process.
...
![The TCP/IP Five-Layer Network Model - Coursera](/../public/images/2023-03-08-Net%20Working/0.jpg)
...
Generic BSD Sockets are more abstract structures that contain INET sockets. Applications read from and write to BSD sockets; the BSD sockets translate the operations into INET socket operations. See [Chapter 4](https://kernelnewbies.org/Documents/LinuxIPNetworking#chapter4) for more on sockets.

Applications, run in user space, form the top level of the protocol stack; they can be as simple as two-way chat connection or as complex as the Routing Information Protocol (RIP - see [Chapter 9](https://kernelnewbies.org/Documents/LinuxIPNetworking#chapter9)).
...
![](/../public/images/2023-03-08-Net%20Working/LinuxIPNetworkingaction=AttachFile&do=get&target=o_skbuff-1677499004880-12.png)
...
<img src="/../public/images/2023-03-08-Net%20Working/LinuxIPNetworkingaction=AttachFile&do=get&target=s_tx-1677499128270-14.png" style="transform:rotate(90deg);"/>
...
<img src="/../public/images/2023-03-08-Net%20Working/LinuxIPNetworkingaction=AttachFile&do=get&target=r_rx-1677499281736-15.png" style="transform:rotate(90deg);"/>
...
See [Figure 7.1](https://kernelnewbies.org/Documents/LinuxIPNetworking#fig7.1) for an abstract diagram of the the forwarding process. (It is essentially a combination of the receiving and sending processes.)



<img src="/../public/images/2023-03-08-Net%20Working/LinuxIPNetworkingaction=AttachFile&do=get&target=f_fwd.png" style="transform:rotate(90deg);"/>
...
![r_overview.png](/../public/images/2023-03-08-Net%20Working/LinuxIPNetworkingaction=AttachFile&do=get&target=r_overview.png)
...
![r_neigh_struct.png](/../public/images/2023-03-08-Net%20Working/LinuxIPNetworkingaction=AttachFile&do=get&target=r_neigh_struct.png)
...
![r_fib_gen.png](/../public/images/2023-03-08-Net%20Working/LinuxIPNetworkingaction=AttachFile&do=get&target=r_fib_gen.png)
...
![r_fib_struct.png](/../public/images/2023-03-08-Net%20Working/LinuxIPNetworkingaction=AttachFile&do=get&target=r_fib_struct.png)
...
![r_cache_gen.png](/../public/images/2023-03-08-Net%20Working/LinuxIPNetworkingaction=AttachFile&do=get&target=r_cache_gen.png)
...
![r_cache_struct.png](/../public/images/2023-03-08-Net%20Working/LinuxIPNetworkingaction=AttachFile&do=get&target=r_cache_struct.png)

Figure 8.6: Routing Cache data structure relationships.

![r_dst_struct.png](/../public/images/2023-03-08-Net%20Working/LinuxIPNetworkingaction=AttachFile&do=get&target=r_dst_struct.png)
...
5. ip_route_output()` updates the statistics in the newfound `dst_cache` structure of the table entry, sets a pointer for the calling fun`ction to refer to the route, and returns a 0 indicating success.
...
![d_rip.png](/../public/images/2023-03-08-Net%20Working/LinuxIPNetworkingaction=AttachFile&do=get&target=d_rip.png)
...
9. Copy the new kernel to the */boot* directory and change the kernel link (usually *vmlinuz*):
   ...
   - `cp arch/i386/boot/bzImage /boot/vmlinuz-2.2.xx` `ln -sf /boot/vmlinuz-2.2.xx /boot/vmlinuz`

10. Copy the new *System.map* file to the */boot* directory and change the map link:

    - `cp System.map /boot/System.map-2.2.xx` `ln -sf /boot/System.map-2.2.xx /boot/System.map`
...
- displays the neighbor table (`arp_tbl`); the IP and hardware addresses, hardware type, device, and flags. (`arp_get_info()` : *net/ipv4/arp.c* 988)
...
![x_setup.png](/../public/images/2023-03-08-Net%20Working/LinuxIPNetworkingaction=AttachFile&do=get&target=x_setup.png)
...
| ![x_pong.png](/../public/images/2023-03-08-Net%20Working/LinuxIPNetworkingaction=AttachFile&do=get&target=x_pong.png) |
| ------------------------------------------------------------ |
...
| ![x_blast.png](/../public/images/2023-03-08-Net%20Working/LinuxIPNetworkingaction=AttachFile&do=get&target=x_blast.png) |
| ------------------------------------------------------------ |
...
Dropping packets from a TCP connection resulted in a fairly linear drop in performance on the ping-pong test; see the graph in [Figure 13.2](https://kernelnewbies.org/Documents/LinuxIPNetworking#fig13.2). This is as expected; when either a packet or acknowledgement is lost, the sender pauses and then sends again. The RTT is also very close (certainly within the expected experimental error) no matter which machine is the "source"; again this is because the benchmark tests the behavior of both machines at the same time.
...
- [http://www.linuxhq.com](http://www.linuxhq.com/)

**Linux HOWTOs**

- [ftp://metalab.unc.edu/pub/Linux/docs/HOWTO](ftp://metalab.unc.edu/pub/Linux/docs/HOWTO)
...
- [http://www.linuxrouter.org](http://www.linuxrouter.org/)
...
- Maxwell, Scott, Coriolis``Open Press, Scottsdale, AZ, 1999.
...
KernelNewbies: Documents/LinuxIPNetworking (2017-12-30 01:30:32由localhost编辑)
